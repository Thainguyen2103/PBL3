...
// ⚠️ Đảm bảo bạn đã có các file này, nếu chưa thì báo mình nhé
...
const getMyReaction = (reactions, userId) => {
    ...
    return myReact ? REACTIONS.find(r => r.id === myReact.type) : null;
};
...
// --- MODALS & STATS ---
const ReactionListModal = ({ reactions, onClose, onUserClick }) => {
            ...
            <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[500px]" onClick={e => e.stopPropagation()}>
                <div className="border-b border-gray-100">
                    <div className="p-4 flex justify-between items-center">
                        <h3 className="font-bold text-slate-800">Cảm xúc về bài viết</h3>
                    ...
                    <div className="flex gap-2 px-4 pb-0 overflow-x-auto custom-scrollbar">
                        <button onClick={() => setActiveTab('all')} className={`pb-3 px-2 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${activeTab === 'all' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:bg-gray-50'}`}>Tất cả <span className="text-xs ml-1 bg-gray-100 px-1.5 rounded-full">{users.length}</span></button>
                        {availableTypes.map(type => {
                            const icon = REACTIONS.find(r => r.id === type)?.icon;
                        ...
                        <div className="space-y-4">
                            {displayList.map(u => (
                                <div key={u.id} className="flex items-center gap-3 cursor-pointer group" onClick={() => { onUserClick && onUserClick(u); onClose(); }}>
                                    <div className="relative"><img src={u.avatar} className="w-10 h-10 rounded-full object-cover border border-gray-100" alt="" /><div className="absolute -bottom-1 -right-1 bg-white rounded-full p-0.5 shadow-sm text-[10px]">{REACTIONS.find(r => r.id === u.reactionType)?.icon}</div></div>
...
const ReactionSummary = ({ reactions, onClick }) => {
    ...
    const tooltipText = sortedTypes.map(t => `${REACTIONS.find(r => r.id === t)?.label || t}: ${counts[t]}`).join('\n');
    return (
        <div className="group/stats relative flex items-center cursor-pointer hover:bg-gray-50 rounded-full pr-2 transition-colors" onClick={(e) => { e.stopPropagation(); onClick(); }}>
            <div className="flex items-center gap-1 bg-white rounded-full px-1 shadow-sm border border-gray-100">
                <div className="flex -space-x-1">{sortedTypes.map(t => <span key={t} className="text-[10px] bg-gray-50 rounded-full w-4 h-4 flex items-center justify-center border border-white relative z-10">{REACTIONS.find(r => r.id === t)?.icon || '👍'}</span>)}</div>
...
const ConfirmDialog = ({ title, message, onConfirm, onCancel }) => (
    <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-black/20 backdrop-blur-[2px] animate-in fade-in duration-200">
        <div className="bg-white rounded-2xl shadow-2xl p-6 w-80 border border-gray-100 transform scale-100 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex gap-3"><button onClick={onCancel} className="flex-1 py-2.5 rounded-xl bg-gray-100 text-gray-700 font-bold text-sm hover:bg-gray-200 transition-colors">Hủy</button><button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl bg-red-500 text-white font-bold text-sm hover:bg-red-600 shadow-lg shadow-red-500/30 transition-all">Xóa ngay</button></div>
        </div>
    </div>
);
...
const ForumTab = ({ user, onUserClick }) => {
    ...
    const handlePost = async () => {
        ...
        if (selectedFiles.length > 0) {
            ...
            catch (error) { alert("Lỗi upload: " + error.message); setPosting(false); return; }
        }
            ...
            <div key={cmt.id} className={`flex gap-2 group ${isReply ? 'ml-10 mt-1' : 'mt-3'}`}>
                ...
                <div className="flex-1">
                    <div className="bg-gray-100 rounded-2xl px-3 py-2 text-sm inline-block relative group/bubble min-w-[150px]">
                        ...
                        {cmtLikes.length > 0 && <div className="absolute -bottom-2 -right-2 bg-white rounded-full px-1 py-0.5 shadow-sm border border-gray-200 flex items-center gap-0.5 z-10"><ReactionSummary reactions={cmtLikes} onClick={() => setViewingReactions(cmtLikes)} /></div>}
                    </div>
                    <div className="flex gap-3 ml-3 mt-1 text-[11px] text-gray-500 font-medium items-center relative">
                        <CustomTooltip content={formatTimeFull(cmt.created_at)}><span className="text-gray-400 cursor-pointer hover:underline">{formatTimeRelative(cmt.created_at)}</span></CustomTooltip>
                        <div className="group/react relative">
                            <button className={`font-bold hover:underline ${myReact ? myReact.color : 'text-gray-500'}`} onClick={() => handleCommentReaction(cmt.id, cmtLikes, post.id, myReact ? myReact.id : 'like')}>{myReact ? myReact.label : 'Thích'}</button>
                            <div className="absolute bottom-full left-0 mb-0 pb-2 hidden group-hover/react:flex flex-col justify-end z-50"><div className="bg-white rounded-full shadow-xl border border-gray-100 p-1 flex gap-1 animate-in zoom-in-95 origin-bottom">{REACTIONS.map(react => <button key={react.id} onClick={(e) => { e.stopPropagation(); handleCommentReaction(cmt.id, cmtLikes, post.id, react.id); }} className="text-lg hover:scale-125 transition-transform p-0.5 origin-bottom" title={react.label}>{react.icon}</button>)}</div></div>
                        </div>
                        <button onClick={() => handleReplyClick(post.id, cmt.id, cmt.users?.full_name)} className="font-bold hover:underline text-gray-500">Phản hồi</button>
                        {isMyComment && <button onClick={() => requestDeleteComment(post.id, cmt.id)} className="font-bold hover:underline text-red-500 hover:bg-red-50 px-1 rounded transition-colors">Xóa</button>}
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="bg-[#f0f2f5] h-full flex flex-col relative z-10 overflow-hidden rounded-[2rem] border border-gray-200 shadow-xl font-sans">
            <div className="bg-white px-5 py-3 border-b border-gray-200 flex justify-between items-center shadow-sm shrink-0 z-20">
                <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">Cộng đồng <span className="text-blue-600">Kanji</span></h2>
                <div className="flex gap-2">
                    <button onClick={() => setShowManager(true)} className="flex items-center gap-2 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 rounded-full text-xs font-bold text-blue-700 border border-blue-200">📝 Quản lý bài viết</button>
                    <button onClick={fetchPosts} className="p-1.5 hover:bg-gray-100 rounded-full text-gray-600 font-bold">↻</button>
                </div>
            </div>
                ...
                <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200">
                    <div className="flex gap-3 mb-2">
                        ...
                        <div className="flex-1">
                            <div className="bg-gray-100 rounded-3xl px-4 py-2 focus-within:ring-2 focus-within:ring-blue-100/50">
                                <textarea ref={textareaRef} value={newContent} onChange={handleTextareaChange} placeholder={`Bạn đang nghĩ gì, ${user?.full_name?.split(' ').pop()}?`} className="w-full bg-transparent border-none outline-none focus:ring-0 p-0 text-[15px] resize-none text-slate-800 caret-blue-600 overflow-hidden" rows={1} style={{ minHeight: '40px' }} />
                            </div>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-gray-50 mt-2 px-1 relative">
                        <div className="flex gap-1">
                             <button onClick={() => fileInputRef.current.click()} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 rounded-lg text-gray-600 text-xs font-bold"><span className="text-green-500 text-lg">🖼️</span> Ảnh/Video</button>
                             <input type="file" ref={fileInputRef} onChange={handleImageSelect} className="hidden" accept="image/*" multiple />
                             <div className="relative">
                                 <button onClick={(e) => { e.stopPropagation(); setShowEmojiPicker(!showEmojiPicker); }} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 rounded-lg text-gray-600 text-xs font-bold"><span className="text-yellow-500 text-lg">😄</span> Cảm xúc</button>
                        ...
                        <button onClick={handlePost} disabled={posting || (!newContent.trim() && selectedFiles.length === 0)} className="px-6 py-2 rounded-xl text-xs font-black bg-blue-600 text-white shadow-md hover:bg-blue-700 disabled:bg-gray-300">{posting ? 'Đang đăng...' : 'ĐĂNG'}</button>
                    </div>
                </div>
                    ...
                    return (
                        <div key={post.id} id={`post-${post.id}`} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-visible hover:shadow-md transition-shadow">
                            <div className="p-4 flex justify-between items-start">
                                <div className="flex gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-gray-500 flex items-center gap-1"><CustomTooltip content={formatTimeFull(post.created_at)}><span className="cursor-pointer hover:underline">{formatTimeRelative(post.created_at)}</span></CustomTooltip><span>•</span><span>🌎</span></div>
                                    </div>
                                </div>
                                <div className="relative">
                                    ...
                                    {activeMenu === post.id && <div className="absolute right-0 top-full mt-1 bg-white shadow-xl rounded-xl border border-gray-100 p-1.5 w-40 z-50 animate-in fade-in zoom-in-95">{user?.id === post.user_id ? <><button onClick={() => { setEditingPost(post); setActiveMenu(null); }} className="w-full text-left px-3 py-2 text-gray-700 hover:bg-gray-50 rounded-lg text-xs font-bold flex items-center gap-2">✏️ Chỉnh sửa</button><button onClick={() => requestDeletePost(post.id)} className="w-full text-left px-3 py-2 text-red-600 hover:bg-red-50 rounded-lg text-xs font-bold flex items-center gap-2">🗑️ Xóa bài</button></> : <button className="w-full text-left px-3 py-2 text-gray-600 hover:bg-gray-50 rounded-lg text-xs font-medium">🚩 Báo cáo</button>}</div>}
                                </div>
                            </div>
                            ...
                            <div className="px-4 py-2 flex justify-between items-center text-gray-500 text-xs mx-1">
                                <div className="flex items-center gap-1 h-5"><ReactionSummary reactions={likes} onClick={() => setViewingReactions(likes)} /></div>
                                <div className="hover:underline cursor-pointer font-medium" onClick={() => setOpenComments(prev => ({...prev, [post.id]: !prev[post.id]}))}>{comments.length > 0 ? `${comments.length} bình luận` : ''}</div>
                            </div>

                            <div className="px-2 py-1 flex border-t border-gray-100 mx-2 mt-1">
                                <div className="flex-1 group relative">
                                    <button onClick={() => handlePostReaction(post.id, likes, myReact ? myReact.id : 'like')} className={`w-full flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold transition-colors ${myReact ? myReact.color : 'text-gray-600 hover:bg-gray-100'}`}><span>{myReact ? myReact.icon : '👍'}</span> {myReact ? myReact.label : 'Thích'}</button>
                                    <div className="absolute bottom-full left-0 w-full flex justify-center pb-2 opacity-0 group-hover:opacity-100 invisible group-hover:visible transition-all duration-200 z-50"><div className="bg-white rounded-full shadow-xl border border-gray-100 p-1.5 flex gap-2 animate-in zoom-in-95 origin-bottom">{REACTIONS.map(react => <button key={react.id} onClick={(e) => { e.stopPropagation(); handlePostReaction(post.id, likes, react.id); }} className="text-2xl hover:scale-125 transition-transform origin-bottom">{react.icon}</button>)}</div></div>
                                </div>
                                <button onClick={() => setOpenComments(prev => ({...prev, [post.id]: !prev[post.id]}))} className="flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-gray-600 hover:bg-gray-100 text-sm font-bold">💬 Bình luận</button>
                            </div>
                                ...
                                <div className="bg-gray-50 px-4 py-3 border-t border-gray-200">
                                    ...
                                    <div className="flex gap-2 items-end">
                                        ...
                                        <div className="flex-1 bg-gray-100 rounded-2xl px-3 py-2 relative transition-all focus-within:ring-1 focus-within:ring-blue-200">
                                            {replyingTo[post.id] && <div className="flex justify-between items-center text-[11px] text-blue-600 bg-blue-50 px-2 py-0.5 rounded-lg mb-1 w-fit"><span>Đang trả lời <b>{replyingTo[post.id].userName}</b></span><button onClick={() => cancelReply(post.id)} className="ml-2 hover:text-red-500 font-bold">✕</button></div>}
                                            ...
                                            <div className="flex items-end gap-2">
                                                <textarea ref={(el) => (inputRefs.current[post.id] = el)} value={commentInputs[post.id] || ''} onChange={(e) => handleCommentInputChange(post.id, e)} onKeyDown={(e) => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSendComment(post.id))} placeholder="Viết bình luận..." className="w-full bg-transparent border-none outline-none text-[13px] text-slate-800 placeholder-gray-500 resize-none overflow-hidden py-1 max-h-32" rows={1} />
            ...
            {viewingReactions && <ReactionListModal reactions={viewingReactions} onClose={() => setViewingReactions(null)} onUserClick={onUserClick} />}
            {editingPost && <EditPostModal post={editingPost} user={user} onClose={() => setEditingPost(null)} onUpdate={handlePostUpdated} />}
            {deleteConfirmId && <ConfirmDialog title="Xóa bài viết?" message="Bài viết này sẽ bị xóa vĩnh viễn và không thể khôi phục." onConfirm={confirmDeletePost} onCancel={() => setDeleteConfirmId(null)} />}
            {deleteCommentInfo && <ConfirmDialog title="Xóa bình luận?" message="Bạn có chắc chắn muốn xóa bình luận này không?" onConfirm={confirmDeleteComment} onCancel={() => setDeleteCommentInfo(null)} />}
