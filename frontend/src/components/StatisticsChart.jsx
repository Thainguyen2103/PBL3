                ...
                <div className="flex-1 flex border-l-2 border-gray-300 bg-white">
                    {data.map((item, idx) => (
                        <div key={idx} className="flex-1 flex flex-col items-center justify-center min-w-0 px-px">
                            <span className={`text-sm font-bold leading-tight ${item.isToday
...
const StatisticsChart = ({
    ...
    const tabs = [
        { key: 'rankPoints', label: t.stats_rank_battle || 'Rank Đấu', color: 'orange', icon: <ChartIcons.Gamepad />, total: monthlyTotals.rankPoints, unit: t.stats_pts || 'pts' },
        { key: 'kanjiLearned', label: t.stats_power || 'Sức mạnh', color: 'cyan', icon: <ChartIcons.Kanji />, total: monthlyTotals.kanjiLearned, unit: t.stats_kanji_unit || 'Hán tự' },
        { key: 'challengeScore', label: t.stats_achievements_label || 'Chiến công', color: 'purple', icon: <ChartIcons.Swords />, total: monthlyTotals.challengeScore, unit: t.stats_score_unit || 'điểm' }
    ];
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-slate-800 to-slate-900 text-white">
                ...
                <div className="text-center">
                    ...
                    <div className="text-sm text-cyan-300 mt-1 font-semibold">
                        {t.stats_chart_total || 'Tổng'}: {currentTab.total} {currentTab.unit}
                    </div>
                </div>
                <button
                    onClick={onNextMonth}
            ...
            <div className="flex border-b border-gray-200 bg-gray-50">
                {tabs.map((tab) => (
                    <button
                        ...
                        className={`flex-1 flex items-center justify-center gap-2 py-3 px-2 text-sm font-bold transition-all border-b-2 ${activeTab === tab.key
            ...
            <div className="flex items-center justify-center gap-4 px-4 py-4 bg-gray-50/50 border-b border-gray-100">
                <span className="text-sm text-gray-600 font-semibold">{t.stats_chart_type || 'Kiểu:'}</span>
                {[
                    { key: 'bar', icon: <ChartIcons.Bar />, label: t.stats_chart_bar || 'Cột' },
                    { key: 'line', icon: <ChartIcons.Line />, label: t.stats_chart_line || 'Đường' },
                    { key: 'spline', icon: <ChartIcons.Spline />, label: t.stats_chart_spline || 'Cong' }
                ].map((type) => (
                    <button
                        key={type.key}
                        onClick={() => setChartType(type.key)}
                        className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${chartType === type.key
            ...
            <div className="px-4 py-4 bg-gray-50 border-t border-gray-100 text-center">
                <p className="text-sm text-gray-600 font-medium flex items-center justify-center gap-2">
                    ...
                    {t.stats_chart_footer || 'Biểu đồ hiển thị số lượng học được trong từng ngày'}
                </p>
            </div>
        </div>
    );
