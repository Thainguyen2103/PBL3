...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Đảm bảo endpoint bắt đầu bằng /api/ để khớp với server.js
    ...
    try {
      ...
      if (response.ok) {
        if (isLogin) {
          // Lưu dữ liệu vào session để các trang sau có thể sử dụng
  ...
  useEffect(() => {
    ...
    const chars = Array.from({ length: 75 }).map((_, i) => ({
      id: i,
      char: fullKanjiList[Math.floor(Math.random() * fullKanjiList.length)],
      left: (i % 30 * 3.3) + "%",
      duration: "40s",
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center bg-gray-100 overflow-hidden font-sans">
      {/* (Phần giao diện UI giữ nguyên theo yêu cầu của bạn) */}
      ...
      <div className="relative z-10 w-full max-w-[420px] flex flex-col items-center px-4">
        ...
        <div className="w-full bg-white px-8 py-8 rounded-[2.5rem] shadow-2xl border border-gray-100 relative z-10">
          ...
          <div className="flex mb-6 border-b border-gray-100">
            <button onClick={() => setIsLogin(true)} className={`w-1/2 pb-2 text-sm font-bold ${isLogin ? 'border-b-2 border-black text-black' : 'text-gray-300'}`}>{t.login}</button>
            <button onClick={() => setIsLogin(false)} className={`w-1/2 pb-2 text-sm font-bold ${!isLogin ? 'border-b-2 border-black text-black' : 'text-gray-300'}`}>{t.register}</button>
          </div>

          {errorMsg && <div className="mb-4 p-2 bg-red-50 text-red-600 text-[10px] font-bold rounded-xl text-center">{errorMsg}</div>}

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1 ml-1">{t.user}</label>
              <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-5 py-2.5 bg-gray-50 border rounded-2xl outline-none" placeholder="example@gmail.com" />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1 ml-1">{t.pass}</label>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-5 py-2.5 bg-gray-50 border rounded-2xl outline-none pr-12" />
            ...
            {!isLogin && (
              <div>
                ...
                <input type="password" required value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="w-full px-5 py-2.5 bg-gray-50 border rounded-2xl outline-none" />
              </div>
            )}
            <button type="submit" className="w-full bg-black text-white font-black py-5 rounded-2xl shadow-xl uppercase tracking-widest text-base mt-2">
