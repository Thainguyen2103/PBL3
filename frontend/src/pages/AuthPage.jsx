...
const AuthPage = () => {
  ...
  // --- HÀM XỬ LÝ SUBMIT (ĐÃ SỬA ĐƯỜNG DẪN /API ĐỂ KHỚP VỚI SERVER) ---
  const handleSubmit = async (e) => {
    ...
    try {
      // QUAN TRỌNG: Thêm tiền tố /api/ để tránh lỗi 404
      ...
      // Xử lý khi Render chưa khởi động xong
      setErrorMsg("Máy chủ đang khởi động, vui lòng đợi 30 giây và thử lại!");
    }
  };
  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center bg-gray-100 overflow-hidden font-sans">
          ...
          {isLangMenuOpen && (
            <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-50 animate-fade-in-down">
              {Object.keys(translations).map((key) => (
                <button key={key} onClick={() => changeLanguage(key)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold ${lang === key ? 'bg-black text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
      ...
      <div className="relative z-10 w-full max-w-[420px] flex flex-col items-center px-4">
        ...
        <div className={`w-full bg-white px-8 ${isLogin ? 'py-8' : 'py-5'} rounded-[2.5rem] shadow-2xl border border-gray-100 relative z-10`}>
