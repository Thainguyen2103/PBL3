...
// 1. IMPORT CONTEXT ĐỂ ĐỒNG BỘ NGÔN NGỮ & LOGIN
import { useAppContext } from '../context/AppContext';

// --- CÁC COMPONENT PHỤ (Notification, Icons, Mascot...) GIỮ NGUYÊN ---
...
const AuthPage = () => {
  ...
  // 2. SỬ DỤNG CONTEXT: Lấy setUser, language, setLanguage và t (bộ từ điển chuẩn)
  ...
  // 3. LOGIC ĐỔI NGÔN NGỮ QUA CONTEXT
  const changeLanguage = (languageKey) => {
    setLanguage(languageKey);           // Cập nhật State toàn App
    localStorage.setItem('appLang', languageKey); // Lưu vào máy
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (isLogin) {
          // --- 4. CẬP NHẬT CONTEXT KHI LOGIN THÀNH CÔNG ---
          // Đảm bảo lưu cả ngôn ngữ hiện tại vào session
          const sessionData = { ...data.session, language: language };
          
          localStorage.setItem('session', JSON.stringify(sessionData));
          setUser(sessionData); // Báo cho App biết đã Login
            ...
            <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-50 animate-fade-in-down">
              {Object.keys(translations).map((key) => (
                <button 
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold ${language === key ? 'bg-black text-white' : 'hover:bg-gray-100 text-gray-700'}`}
                >
      ...
      <div className="relative z-10 w-full max-w-[420px] flex flex-col items-center px-4">
        ...
        <div className={`w-full bg-white px-8 ${isLogin ? 'py-8' : 'py-5'} rounded-[2.5rem] shadow-2xl border border-gray-100 relative z-10 transition-all`}>
          ...
          <div className={`text-center ${isLogin ? 'mb-6' : 'mb-3'} mt-2`}>
            {/* Dùng t từ Context để hiển thị chữ */}
