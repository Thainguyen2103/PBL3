...
import { translations } from '../utils/translations'; // Đảm bảo bạn vẫn giữ file này
import { getKanjiList } from '../utils/kanjiData';   // Đảm bảo bạn vẫn giữ file này

// --- 1. CÁC COMPONENT PHỤ & ICON (GIỮ NGUYÊN) ---

// Component Thông báo (Toast) - Mới thêm để hiển thị kết quả đẹp mắt
...
const AuthPage = () => {
  ...
  const [errorMsg, setErrorMsg] = useState(''); // Để chạy hiệu ứng rung lắc (Shake)
  ...
  // Logic kiểm tra độ mạnh mật khẩu (Giữ nguyên)
  ...
  // Hiệu ứng Kanji bay (useEffect cũ)
  useEffect(() => {
  ...
  // --- LOGIC XỬ LÝ SUBMIT (KẾT NỐI SERVER THẬT) ---
  const handleSubmit = async (e) => {
    e.preventDefault();
    setErrorMsg(''); // Reset lỗi rung lắc
    setIsLoading(true);
    ...
    try {
      // GỌI API SERVER RENDER (đã nối Supabase)
      ...
      if (response.ok) {
        ...
        if (isLogin) {
          // Lưu token thật từ Supabase
          localStorage.setItem('session', JSON.stringify(data.session));
          // Chuyển trang sau 1.5s để người dùng kịp đọc thông báo
          setTimeout(() => navigate('/home'), 1500);
        } else {
          // Đăng ký xong thì chuyển qua tab Đăng nhập
          setTimeout(() => setIsLogin(true), 1500);
        }
      } else {
        // --- THẤT BẠI (Backend trả lỗi 400/401) ---
        setErrorMsg(data.error || "Lỗi không xác định"); // Kích hoạt hiệu ứng rung
        showToast(data.error || "Thất bại", "error"); // Hiện Toast đỏ
      }
    } catch (err) {
      // Lỗi mạng hoặc Server chưa bật
      setErrorMsg("Không kết nối được Server!");
      showToast("Server đang khởi động, vui lòng thử lại sau 30s!", "error");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className={`w-full bg-white px-8 ${isLogin ? 'py-8' : 'py-5'} rounded-[2.5rem] shadow-2xl border border-gray-100 relative z-10 transition-all`}>
          ...
          <form onSubmit={handleSubmit} className={`space-y-${isLogin ? '4' : '3'}`}>
            ...
            <div className="relative">
              ...
              {/* THANH ĐÁNH GIÁ MẬT KHẨU (GIỮ NGUYÊN) */}
            ...
            {isLogin ? (
              <div className="flex justify-between items-center text-xs px-1 font-medium pb-1">
                ...
                <button type="button" className="text-gray-400 hover:text-black font-bold">{t.forgot}</button>
              </div>
            ) : (
